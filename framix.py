...
from engine.tinker import (
    Craft, Search, Active, Review, FileAssist, FramixError
)
from nexacore.api import Api
...
class Missions(object):
    ...
    async def painting(self, option: "Option", deploy: "Deploy") -> None:
        ...
        -------
        list[Any] or None
        - 若绘图成功，返回一个包含绘制结果或渲染组件的列表；
        - 若流程中断或未能生成任何内容，返回 None。
        ...
        async def load_fully(fully: str) -> dict:
            ...
            try:
                ...
                if Path(fully).is_file():
                    data = await FileAssist.load_parameters(fully)
                else:
                    data = await Api.profession(fully)
