...
### ⚜️ 频率探测 (`--frate`)
#### 📔 功能描述:
- 控制视频处理过程中的帧率采样频率，平衡画面流畅度与计算资源消耗。
...
```
    结构相似度 (SSIM) 连续帧变化示意图（threshold = 0.95）
...
| 场景描述        | 推荐取值          | 说明                          |
|-------------|---------------|-----------------------------|
| 极高精度匹配      | `0.98 ~ 0.99` | 适用于图像非常稳定、无压缩噪声或几乎无变化的视频。   |
| 一般精度匹配      | `0.95 ~ 0.98` | 推荐范围，适合大多数高清、正常质量视频。        |
| 容错性检测（冗余处理） | `0.90 ~ 0.95` | 当视频存在轻微变化或压缩噪声时使用，避免误判为不稳定。 |
| 快速检测、容差较高   | `< 0.90`      | 允许较大波动，适用于快速粗粒度过滤。          |

- 若未设置，`thres` 默认为 `const.THRES`，通常为 `0.98`。
- 配合 `psnr_threshold` 使用可进一步增强稳定性判断的保守性。
- 实际项目中建议通过可视化统计获取最优值。
#### 📔 应用场景:
...
#### 📔 参数说明: 
- 一个可选的整数。
- 对视频中 **轻微跳帧或编辑点模糊** 的段落，推荐设置 `shift=1~2`。
- 对于要求精确分析的场景（如转场检测），建议使用默认值 `shift=0`。
...
- **稳定段过滤**: `grade=1~2` 在判断稳定阶段时，更好兼顾远近影响。
#### 📔 实际应用: 
```
framix --grade 2
```
