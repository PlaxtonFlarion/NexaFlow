...
```
┌────────────┐     ┌───────────────┐     ┌──────────────────┐
│  --speed   │ ──▶ │ ID            │ ──▶ │ Report engine    │
└────────────┘     └───────────────┘     └──────────────────┘

┌────────────┐     ┌───────────────┐     ┌──────────────────┐     ┌──────────────────┐     ┌───────────────┐
│  --basic   │ ──▶ │ ID Timestamps │ ──▶ │ Timing structure │ ──▶ │ Timing alignment │ ──▶ │ Report engine │
└────────────┘     └───────────────┘     └──────────────────┘     └──────────────────┘     └───────────────┘

┌────────────┐     ┌───────────────┐     ┌──────────────────┐     ┌──────────────────┐     ┌───────────────┐     ┌───────────────────────────┐     ┌───────────────┐
│  --keras   │ ──▶ │ ID Timestamps │ ──▶ │ Timing structure │ ──▶ │ Timing alignment │ ──▶ │ Deep learning │ ──▶ │ High-level semantic graph │ ──▶ │ Report engine │
└────────────┘     └───────────────┘     └──────────────────┘     └──────────────────┘     └───────────────┘     └───────────────────────────┘     └───────────────┘
```
...
#### 📔 参数说明: 
- 一个可选的浮点数，按比例调整视频帧的尺寸，默认值 0.5（等比缩小为原始尺寸的一半），取值范围 0.0 ～ 1.0。
...
#### 📔 参数说明: 
- 一个可选的浮点数，默认值为 0.98，取值范围 0.00 ~ 1.00（实际项目中建议通过可视化统计获取最优值）。
- 一般设置在 0.85 ~ 0.99 之间。
  - 数值越大
    - 判定为“稳定”的门槛更高。
...
- 偏移值越小，相邻窗口之间重合度越高，有助于捕捉细微变动；偏移值越大，则跳跃处理更强，适用于高节奏场景分析。
