...
### ⚜️ 凝滞核心 (`--delay`)
#### 📔 功能描述:
- 启动 **凝滞核心机制**，在执行主流程前注入指定的延迟时长（单位：秒）。
- 用于模拟冷启动等待、手动干预窗口、环境初始化缓冲等场景。
- 保证操作节奏一致性，为后续任务铺垫稳定执行时机。
#### 📔 参数说明:
- 一个 **正整数**，单位为 **秒**，默认为`0`（没有延时）。
- 不支持小数或时间字符串，仅接受如 `1`、`5`、`30` 这类纯秒数形式。
...
[--delay 0.5] ❌ 无效（仅支持正整数）
[--delay 00:00:05] ❌ 无效（无需时间格式）

→ 参数必须是正整数，如 3、10、60 等。
→ 自动阻塞主流程，倒计时结束后自动启动主任务。
```

#### 📔 应用场景:
- **冷启动等待**：为被测应用、脚本或录屏工具预留启动时间。
- **外设初始化**：确保设备就绪（如镜像、连接、UI加载）后再启动。
- **任务节奏控制**：用于多设备联动、多流程串联时的统一节奏管理。
- **演示预热**：为操作者留出准备时间，避免操作冲突。
...
#### 📔 参数说明:
- 一个可选的整数，表示目标帧率（Frames Per Second, FPS），默认值为 60，取值范围 30 ~ unlimited。
