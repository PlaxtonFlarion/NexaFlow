...
class Missions(object):
    ...
    def __init__(self, fully: bool, alone: bool, quick: bool, basic: bool, keras: bool, group: bool, *args, **kwargs):
        self.fully, self.alone, self.quick, self.basic, self.keras, self.group = fully, alone, quick, basic, keras, group
    ...
    async def analysis(self, deploy: Deploy):
        ...
        async def load_commands():
            try:
                async with aiofiles.open(script_file, "r", encoding="utf-8") as f:
                    ...
                    exec_list = [
                        {
                            "name": cmds["name"], "loop": cmds["loop"], "time": cmds["time"],
                            "actions": cmds["actions"]
                        } for cmds in json.loads(data)["commands"]
                    ]
        ...
        async def exec_commands():
            ...
            for method in script["actions"]:
                if callable(device_method := getattr(device, method["command"], None)):
                    result_list = await asyncio.gather(
                        *(device_method(*method["args"]) for _ in device_list), return_exceptions=True
                    )
                    for result in result_list:
                        logger.info(result)

        async def script_commence():
        ...
        async def script_over():
            ...
            if not deploy.alone and len(task_list) > 1:
                duration_list = await asyncio.gather(
                    *(ask_video_length(self.ffprobe, temp_video) for temp_video, *_ in task_list)
                )
                duration_list = [duration for duration in duration_list if not isinstance(duration, Exception)]
        ...
        if self.fully:
            script_file = "/Users/acekeppel/PycharmProjects/NexaFlow/data/script.json"
            script_data = await load_commands()
            ...
            for script in script_data:
                ...
                for _ in range(script["loop"]):
                    ...
                    try:
                        ...
                        exec_task = asyncio.create_task(exec_commands())
                        await asyncio.gather(
                            *(timepiece(timer_mode, serial, events)
                              for serial, events in device_events.items())
                        )
...
if __name__ == '__main__':
    ...
    missions = Missions(
        _fully, _alone, _quick, _basic, _keras, _group,
